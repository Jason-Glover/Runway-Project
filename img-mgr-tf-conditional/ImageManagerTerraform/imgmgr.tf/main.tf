...
###############################################
# Dev Autoscaling Group
###############################################

resource "aws_autoscaling_group" "ASGDev" {
  count = terraform.workspace == "dev" ? 1 : 0
  name                      = "${terraform.workspace}-${var.ApplicationName}-ASG"
  max_size                  = var.ASGMaxDev
  min_size                  = var.ASGMinDev
  health_check_grace_period = var.ASG_HC_GracePeriod
  health_check_type         = var.ASGHealthCheck
  desired_capacity          = var.ASGDesiredDev
  vpc_zone_identifier       = data.terraform_remote_state.remote_state.outputs.Private_Subnets
  target_group_arns         = [aws_lb_target_group.alb_target.arn]
  launch_template {
    id      = aws_launch_template.ASG_LTDev[count.index].id
    version = aws_launch_template.ASG_LTDev[count.index].latest_version
  }
  instance_refresh {
    strategy = "Rolling"
...
###############################################
# Dev ASG Launch Template
###############################################

resource "aws_launch_template" "ASG_LTDev" {
  count = terraform.workspace == "dev" ? 1 : 0
  name                   = "${terraform.workspace}-${var.ApplicationName}-LaunchTemplate"
  image_id               = var.ami_id
  instance_type          = var.instance_typeDev
  key_name               = var.SSH_Key
  vpc_security_group_ids = [aws_security_group.ec2.id]
  update_default_version = true
  user_data              = base64encode(local.UserData)  # userdata found in userdata.tf
...
###############################################
# Prod ASG CPU High Scaling Policy
###############################################
...
###############################################
# Prod ASG CPU Low Scaling Policy
###############################################
...
##################################################
# Prod CPU High Alarm for CPU High Scaling Policy
##################################################
...
resource "aws_cloudwatch_metric_alarm" "cpu_high" {
  ...
  alarm_actions     = [
    aws_autoscaling_policy.asg_cpu_high[count.index].arn,
    data.aws_cloudformation_stack.snsarn.outputs.SNSTopicArn
    ]
}

###############################################
# Prod CPU Low Alarm for CPU Low Scaling Policy
###############################################
...
resource "aws_cloudwatch_metric_alarm" "cpu_low" {
  ...
  alarm_actions     = [aws_autoscaling_policy.asg_cpu_low[count.index].arn]
}

###############################################
# Dev ASG CPU High Scaling Policy
###############################################

resource "aws_autoscaling_policy" "asg_cpu_highDev" {
  count = terraform.workspace == "dev" ? 1 : 0
  name                   = "${terraform.workspace}-${var.ApplicationName}-ASG-CPUHigh-SP"
  scaling_adjustment     = 1
  adjustment_type        = "ChangeInCapacity"
  cooldown               = 300
  autoscaling_group_name = aws_autoscaling_group.ASGDev[count.index].name
}

###############################################
# Dev ASG CPU Low Scaling Policy
###############################################

resource "aws_autoscaling_policy" "asg_cpu_lowDev" {
  count = terraform.workspace == "dev" ? 1 : 0
  name                   = "${terraform.workspace}-${var.ApplicationName}-CPULow-SP"
  scaling_adjustment     = -1
  adjustment_type        = "ChangeInCapacity"
...
###############################################
# Dev CPU High Alarm for CPU High Scaling Policy
###############################################

resource "aws_cloudwatch_metric_alarm" "cpu_highDev" {
  count = terraform.workspace == "dev" ? 1 : 0
  alarm_name          = "${terraform.workspace}-${var.ApplicationName}-CPUHigh-Alarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  ...
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.ASGDev[count.index].name
  }

  alarm_description = "This metric monitors ec2 ASG high cpu utilization"
  alarm_actions     = [
    aws_autoscaling_policy.asg_cpu_highDev[count.index].arn,
    data.aws_cloudformation_stack.snsarn.outputs.SNSTopicArn
    ]
}

###############################################
# Dev CPU Low Alarm for CPU Low Scaling Policy
###############################################

resource "aws_cloudwatch_metric_alarm" "cpu_lowDev" {
  count = terraform.workspace == "dev" ? 1 : 0
  alarm_name          = "${terraform.workspace}-${var.ApplicationName}-CPULow-Alarm"
  comparison_operator = "LessThanOrEqualToThreshold"
  evaluation_periods  = "1"
  ...
  alarm_description = "This metric monitors ec2 ASG low cpu utilization"
  alarm_actions     = [aws_autoscaling_policy.asg_cpu_lowDev[count.index].arn]
}
